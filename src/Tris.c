...
Map xtgen(const Points extra)
{
    ...
    const Points ps = prand(w, h, rooms, grid, border, extra);
    const Tris tris = delaunay(ps, w, h, 9 * rooms, flags);
    const Tris edges = ecollect(tsnew(27 * rooms), tris, flags);
    revdel(edges, w, h, flags);
    const Points trapdoors = xpsrandpop(ps, ntraps);
    // Half the remaining rooms (not including trapdoor rooms) are special rooms of interest.
    const Points interests = xpsrandpop(ps, ps.count / 2);
    const Map map = xmgen(h, w, trapdoors, interests);
    mdups(edges, flags);
