...
Hero_Move(Hero hero, const Map map)
{
    ...
    SDL_PumpEvents();
    // <CTRL+D> to exit
    if(key[SDL_SCANCODE_LCTRL] && key[SDL_SCANCODE_D]) hero.health = -1.f;
    if(key[SDL_SCANCODE_RCTRL] && key[SDL_SCANCODE_D]) hero.health = -1.f;
    // Hero rotation <h, l>
    if(key[SDL_SCANCODE_H]) hero.theta -= hero.d0;
    if(key[SDL_SCANCODE_L]) hero.theta += hero.d0;
    // Hero movement <w, a, s, d>
    Hero temp = hero;
    const Point direction = { hero.dx * cosf(hero.theta), hero.dy * sinf(hero.theta) };
