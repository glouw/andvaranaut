...
Hero_Move(Hero hero)
{
    ...
    SDL_PumpEvents();
    // <CTRL+D> to exit
    if(key[SDL_SCANCODE_LCTRL] && key[SDL_SCANCODE_D]) exit(1);
    if(key[SDL_SCANCODE_RCTRL] && key[SDL_SCANCODE_D]) exit(1);
    // Hero rotation <h, l>
    if(key[SDL_SCANCODE_H]) hero.theta -= hero.d0;
    if(key[SDL_SCANCODE_L]) hero.theta += hero.d0;
    // Hero movement <w, a, s, d>
    Hero temp = hero;
    const struct point direction = { hero.dx * cos(hero.theta), hero.dy * sin(hero.theta) };
    ...
    return Geom_ecoll(temp.where, Map_walling) ? hero : temp;
}
