...
int xbraster(void* const bundle)
{
    ...
    {
        // For colums <a> to <b> of the display, all hits of the walling, ceiling, and flooring
        // are collected up until the first eye level wall. The projections
        ...
        b->zbuff[x] = xraster(scanline, hits, b->hero, b->current, b->map);
    }
    return 0;
}
